        ...
        <div class="slider-main" id="sliderMain">
          <?php if (!empty($product_images)): ?>
            ...
            <div class="slide <?= $i === 0 ? 'active' : '' ?>" data-index="<?= $i ?>">
              <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($product['name']) ?>" onclick="openLightbox(this.src)"/>
            </div>
            <?php endforeach; ?>
          <?php else: ?>
    ...
    <div class="products-grid">
      ...
      <a href="product.php?id=<?= $p['id'] ?>" class="product-card">
        <div class="card-img">
          ...
          if ($first_img): ?>
            <img src="<?= htmlspecialchars(trim($first_img)) ?>" alt="<?= htmlspecialchars($p['name']) ?>"/>
          <?php else: ?>
            <?= $icons[$p['category']] ?? '📦' ?>
          <?php endif; ?>
        </div>
