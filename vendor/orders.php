  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    @media(max-width:768px){
      .main{margin-top:65px;padding:1.2rem 4vw;}
    ...
    /* ═══ HAMBURGER + SIDEBAR ═══ */
    .hamburger{display:none;flex-direction:column;gap:5px;cursor:pointer;background:none;border:none;padding:6px;z-index:1001;flex-shrink:0;}
    ...
    @media(max-width:860px){
      nav{height:65px;flex-wrap:nowrap;}
      .hamburger{display:flex;}
      .nav-right{
        position:fixed;top:0;right:-275px;width:260px;height:100vh;
        background:#150a02;flex-direction:column;align-items:stretch;
        padding:75px 1.2rem 2rem;gap:0.4rem;
        z-index:1000;
      ...
      <div class="order-body">
        <div class="product-img-wrap">
          <?php if ($prod_first_img): ?>
            <img src="../<?= htmlspecialchars($prod_first_img) ?>" alt="" onerror="this.style.display='none'"/>
          <?php else: ?>
...
(function() {
  ...
  hamburger.addEventListener('click', function(e) {
    e.stopPropagation();
    if (navRight.classList.contains('open')) { closeNav(); } else { openNav(); }
  });

  // Overlay click => close
  navOverlay.addEventListener('click', closeNav);

  // Sidebar ke andar click propagation nahi rokna — links normally kaam karen
  // (stopPropagation hataya, isliye links directly navigate karenge)
...
function generateTracking(courier) {
  ...
  if (!courier) {
    input.value = '';
    input.placeholder = 'Courier select karo — auto generate hoga';
    return;
  }
  var cfg = courierPrefixes[courier] || { prefix: 'TK', len: 10, suffix: '' };
  var digits = cfg.len - cfg.prefix.length - cfg.suffix.length;
  var num = '';
  for (var i = 0; i < digits; i++) num += Math.floor(Math.random() * 10);
...
function openPrintSlip(order) {
  ...
  setTimeout(function() {
    ...
    var qrHtml = qrSrc ? '<img src="' + qrSrc + '" width="90" height="90" style="display:block;"/>' : '';
    var trackingHtml = order.tracking
      ? '<div style="padding:6px 12px;border-bottom:1px solid #ddd;display:flex;gap:20px;background:#f0ecff;">'
        + '<div><div style="font-size:7px;color:#44337a;font-weight:700;text-transform:uppercase;">COURIER</div>'
        + '<div style="font-size:11px;font-weight:700;">' + order.courier + '</div></div>'
        + '<div><div style="font-size:7px;color:#44337a;font-weight:700;text-transform:uppercase;">TRACKING #</div>'
        + '<div style="font-size:11px;font-weight:700;">' + order.tracking + '</div></div></div>'
      : '';

    var orderId = String(order.id);
    while (orderId.length < 5) orderId = '0' + orderId;

    var slip = '<div id="slipInner" style="font-family:Arial,sans-serif;background:white;">'
      ...
      + '<div style="color:#aaa;font-size:8px;letter-spacing:2px;font-weight:700;">SHIPPING LABEL</div></div>'
      ...
      + '<div style="font-size:20px;font-weight:900;color:#000;">#TK' + orderId + '</div></div>'
      + '<div style="text-align:right;"><div style="font-size:7px;color:#666;font-weight:700;text-transform:uppercase;">DATE</div>'
      + '<div style="font-size:11px;font-weight:700;">' + order.date + '</div></div></div>'
      ...
      + '<div style="font-size:11px;font-weight:700;">TrenzoKart Fulfillment Center</div></div>'
      + '<div style="padding:8px 12px;border-bottom:2px solid #000;display:flex;gap:10px;align-items:flex-start;">'
      + '<div style="flex:1;"><div style="font-size:7px;color:#666;font-weight:700;text-transform:uppercase;margin-bottom:4px;">SHIP TO</div>'
      ...
      + '<div style="font-size:11px;color:#333;line-height:1.5;">📍 ' + order.address + '</div></div>'
      + '<div style="text-align:center;flex-shrink:0;">'
      + '<div style="font-size:7px;color:#666;font-weight:700;letter-spacing:1px;margin-bottom:3px;">📞 CALL QR</div>'
      + qrHtml
      + '<div style="font-size:7px;color:#999;margin-top:2px;">Scan to Call</div></div></div>'
      ...
      + '<div><div style="font-size:7px;color:#666;font-weight:700;text-transform:uppercase;">PRODUCT</div>'
      + '<div style="font-size:12px;font-weight:700;">' + order.product + '</div></div>'
      + '<div style="text-align:right;"><div style="font-size:15px;font-weight:900;color:#000;">₹' + order.amount + '</div>'
      + '<span style="font-size:9px;font-weight:700;color:#c00;background:#ffe;padding:2px 6px;border:1px solid #c00;border-radius:3px;">' + order.payment + '</span></div></div>'
      + trackingHtml
      + '<div style="background:#1a0f02;padding:6px 12px;text-align:center;">'
      + '<div style="font-size:8px;color:#aaa;letter-spacing:1px;">THANK YOU FOR SHOPPING WITH TRENZOKART 🛍️</div></div></div>';
...
function printSlip() {
  var content = document.getElementById('slipInner').outerHTML;
  var win = window.open('', '_blank', 'width=450,height=680');
  var html = '<!DOCTYPE html>'
    + '<html><head><meta charset="UTF-8"/><title>Shipping Label</title>'
    + '<style>*{box-sizing:border-box;margin:0;padding:0;}'
    + 'body{font-family:Arial,sans-serif;background:white;}'
    + '@page{size:4in 6in;margin:0;}'
    + '@media print{html,body{width:4in;height:6in;overflow:hidden;}}'
    + '</style></head><body>'
    + content
    + '</body></html>';
